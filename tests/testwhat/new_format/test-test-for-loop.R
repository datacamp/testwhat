scen <- list(list(type = "NormalExercise", student = "\n  for (i in 1:10) {\n    rpois(10,i)\n  }", solution = "\n  for (i in 1:3) {\n    rnorm(10,i)\n  }", 
    pass = list(test_basic_check_pass_1 = list(long = "check whether basic test works - pass 1", sct = "test_for_loop()"))), 
    list(type = "NormalExercise", solution = "\n  for (i in 1:3) {\n    rnorm(10,i)\n  }", fail = list(test_basic_check_fails_fail_1 = list(long = "check whether basic test fails if there is no for statement - fail 1", 
        sct = "test_for_loop()"))), list(type = "NormalExercise", student = "\n  for (i in 1:10) {\n    rpois(10,i)\n  }", 
        solution = "\n  for (i in 1:10) {\n    rpois(10,i)\n  }", pass = list(test_cond_test_pass_1 = list(long = "check if cond_test is checked correctly - pass 1", 
            sct = c("test_for_loop(cond_test = {", "    test_object(\"i\")", "})")))), list(type = "NormalExercise", 
        student = "\n  for (i in 1:10) {\n    rpois(10,i)\n  }", solution = "\n  for (i in 1:3) {\n    rpois(10,i)\n  }", 
        fail = list(test_cond_test_fails_fail_1 = list(long = "check if cond_test fails correctly - fail 1", 
            sct = c("test_for_loop(cond_test = {", "    test_object(\"i\")", "})")), test_cond_test_fails_fail_2 = list(long = "check if cond_test fails correctly - fail 2", 
            sct = c("test_for_loop(cond_test = {", "    test_object(\"n\")", "})")), test_cond_test_msg_fail_1 = list(long = "check if cond_test fail message is displayed correctly - fail 1", 
            sct = c("test_for_loop(cond_test = {", "    test_object(\"i\", incorrect_msg = \"Wrong condition\")", 
                "})"), message = "Wrong condition"))), list(type = "NormalExercise", student = "\n  for (i in 1:10) {\n    rpois(10,i)\n  }", 
        solution = "\n  for (i in 1:10) {\n    rpois(10,i)\n  }", pass = list(test_expr_test_pass_1 = list(long = "check if expr_test is checked correctly - pass 1", 
