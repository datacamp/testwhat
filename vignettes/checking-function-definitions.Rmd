...
**Note:** in addition to `check_result()`, you can also pipe the result of `check_call()` into `check_output()` and `check_error()`. These functions will respectively check the output and error the function call generated when executed in the student/solution environment.
