...
In R, there are many ways to solve a problem. Writing an SCT that checks when students make a mistake, but is robust to multiple different solutions is a challenge. `check_or` and `check_or` allow you to add logic to your SCT. Instead of running all tests and failing as soon as one of the tests fail, you can conditionally execute tests, and interpret the results separately.
...
You can consider `check_or()` a logic-inducing function. The different calls to `testwhat` functions that are in your SCT are actually all tests that _have_ to pass: they are `AND` tests. With `check_or()` you can add chunks of `OR` tests in there.
